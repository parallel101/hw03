...
auto operator+(std::variant<Ts...> const& a, TR const& b) {  //variant对象+对象，要求variant的每个子类型和对象都是可加的
	using NewRes = std::variant<std::decay_t<decltype(std::declval<Ts>() + std::declval<TR>())>...>;  //variant通过加法新生成的类型

	using Res = decltype(make_unique_typeset(concat(
		std::declval<std::variant<Ts...>>(),
		std::declval<NewRes>()
	)));   //variant新旧类型进行拼接，然后去重  //variant新旧类型进行拼接，然后去重

	return std::visit([&](auto&& arg) -> Res {return arg + b; }, a);
...
auto operator+(TL const& a, std::variant<Ts...> const& b) {  //对象+variant对象，要求对象和variant的每个子类型都是可加的
	using NewRes = std::variant<std::decay_t<decltype(std::declval<TL>() + std::declval<Ts>())>...>;  //variant通过加法新生成的类型

	using Res = decltype(make_unique_typeset(concat(
		std::declval<std::variant<Ts...>>(),
		std::declval<NewRes>()
	)));   //variant新旧类型进行拼接，然后去重

	return std::visit([&](auto&& arg) -> Res {return a + arg; }, b);
	// 请实现自动匹配容器中具体类型的加法！10 分
}
...
auto operator+(std::variant<TLs...> const& a, std::variant<TRs...> const& b) { //两个variant对象相加，要求第一个variant的任意一个子类型与第二个variant的任意一个子类型可加
	using Res = decltype(make_unique_typeset(concat(
		a, b,
		(std::declval<TLs>() + std::declval<std::variant<TRs...>>())...,
		(std::declval<std::variant<TLs...>>() + std::declval<TRs>())...
	)));  //variant新旧类型进行拼接，然后去重

	return std::visit([&](auto&& arg1, auto&& arg2)->Res { return arg1 + arg2; }, a, b);
	// 请实现自动匹配容器中具体类型的加法！10 分
}
